...
if (isset($_COOKIE["paadhaar"])) {
	$paadhaar_num = $_COOKIE["paadhaar"];
	$_view = true;
} else {
	$_view = false;
}
...
if ($_view) {
	...
	if (mysqli_num_rows($result) == 0) {
		...
		$sql = "SELECT * FROM user_details WHERE aadhaar_num='{$paadhaar_num}'";
		$result = mysqli_query($conn, $sql);
		$puser = mysqli_fetch_array($result);
	} else {
		$puser = mysqli_fetch_array($result);
	}
}
...
<nav id="sidebar" class="sidebar-wrapper" <?php if (isset($hideSideNav) && $hideSideNav) echo 'style="display:none"'; ?>>
	<div class="sidebar-content">
		...
		<!-- sidebar-header  -->
		<?php if ($_view) { ?>
			<div style="margin-left: 24px;margin-bottom: 11px;">
				<button type="button" class="btn btn-primary">
					Viewing <?php echo $paadhaar_num ?><span onclick="backToMyProfile()" class="badge badge-light">X</span>
				</button>
			</div>
		<?php } else if ($user_auth['role'] == 'doctor') { ?>
